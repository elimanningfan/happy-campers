...
export const filterRVs = (filters) => {
  return rvData.filter(rv => {
    ...
    if (filters.length && rv.length > filters.length) return false;
    return true;
  });
};

export const searchRVs = (query) => {
...
export const getRVBySlug = (slug) => {
  return rvData.find(rv => rv.slug === slug);
};

export const getFeaturedRVs = () => {
  return rvData.filter(rv => rv.featured);
};

export const getRVsByType = (type) => {
